...
Astro 6 генерирует статический RO/RU-сайт в `dist/`. Румынский язык расположен в корне, русский — в `/ru/`. Статьи хранятся в Markdown и локально редактируются через Keystatic. Страницы моделей формируются из поля `model`. Production-публикация подготовлена через GitHub Actions и FTP, но не выполнялась.
